...
class FullTrajectoryAgent:
    def __init__(self, checkpoint_path, device='cuda'):
        ...
        for k, v in state_dict.items():
            ...
            # [关键] 跳过 spatial_softmax 的 buffer
            # 因为它们是动态生成的，且 checkpoint 中的尺寸 (49) 与初始化尺寸 (1) 不匹配
            # 只要跑一次前向传播，它们就会自动恢复，无需加载
            if "spatial_softmax.pos_x" in new_key or "spatial_softmax.pos_y" in new_key:
                continue
                
            new_dict[new_key] = v
            
        # strict=False 允许缺少部分 buffer (即我们刚刚跳过的那些)
...
def eval_worker(worker_id, checkpoint_path, num_episodes, start_seed, save_video, max_steps, device):
    ...
    try:
        ...
        # 确保这里的 image_width/height 与 data_collector 训练时一致 (96 or 112)
        # 根据你最后一次采集数据时的设置，如果是96请改成96
        data_cfg = DataCollectionConfig(
            ...
            include_object_relative_pos=False # 确保测评也不作弊
        )
