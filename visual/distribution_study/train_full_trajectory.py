...
CONFIG = {
    ...
    # 训练参数
    'batch_size': 13,     # 全轨迹训练显存消耗大，16是一个平衡点，24G显存可尝试32
...
class SpatialSoftmax(nn.Module):
    ...
    def forward(self, feature):
        ...
        # 如果输入特征图尺寸和初始化不一样，动态重新生成网格（增加鲁棒性）
        if H != self.height or W != self.width:
            ...
            self.pos_x = torch.from_numpy(pos_x.reshape(H*W)).float().to(device)
            self.pos_y = torch.from_numpy(pos_y.reshape(H*W)).float().to(device)
            self.height = H
            self.width = W
        ...
        return torch.cat([expected_x, expected_y], dim=-1).view(N, -1)

class ResNetVisualEncoder(nn.Module):
    """
    [修复版] 
    1. 接受 feature_dim 参数
    2. 增加 Projection 层 (256 -> feature_dim)
    """
    def __init__(self, input_channels, feature_dim=64, dropout_prob=0.0, pretrained=True):
        super().__init__()
        ...
        # 修改第一层适应 input_channels (RGBD=4)
        self.conv1 = nn.Conv2d(input_channels, 64, kernel_size=7, stride=2, padding=3, bias=False)
        
        if pretrained and input_channels != 3:
            with torch.no_grad():
                original_weights = resnet.conv1.weight
                self.conv1.weight[:, :3] = original_weights
                if input_channels > 3:
                    self.conv1.weight[:, 3:] = torch.mean(original_weights, dim=1, keepdim=True)
        ...
        # 去掉 layer4 以保持较大的空间分辨率
        self.dropout = nn.Dropout2d(p=dropout_prob)
        # [新增] 投影层：将 ResNet Layer3 的 256 通道降维到 feature_dim (64)
        self.projection = nn.Conv2d(256, feature_dim, kernel_size=1)
        
        # 初始化 Spatial Softmax (尺寸会在 forward 时自动适配)
        self.spatial_softmax = SpatialSoftmax(1, 1) 
        
        self.output_dim = feature_dim * 2 # 输出坐标 (x, y)
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        x = self.layer2(x)
        x = self.layer3(x) # Output channels: 256
        x = self.dropout(x)
        # [新增] 降维
...
class FullTrajectoryPolicy(nn.Module):
    ...
    def __init__(self, config, aux_dim, num_phases=7):
        ...
        cnn_drop = config.get('cnn_dropout', 0.0)
        # 这里的 feature_dim 指的是每个 channel spatial softmax 之前的通道数
        self.encoder = ResNetVisualEncoder(input_channels=in_ch, feature_dim=64, dropout_prob=cnn_drop)
    ...
    def apply_augmentation(self, x):
        ...
        if CONFIG.get('aug_noise', 0) > 0:
            noise = torch.randn_like(x) * CONFIG['aug_noise']
            x = x + noise
            x = torch.clamp(x, 0, 1)
...
def train():
    ...
    train_loader = DataLoader(train_ds, batch_size=CONFIG['batch_size'], shuffle=True, 
                              num_workers=16, collate_fn=collate_fn_pad, pin_memory=True, persistent_workers=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_path", type=str, default='data_enhanced/basket_demos_with_phase.h5')
    parser.add_argument("--save_dir", type=str, default='checkpoints_full_traj_4090')
