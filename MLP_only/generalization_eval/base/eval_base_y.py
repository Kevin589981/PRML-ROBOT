...
def main() -> None:
    ...
    run_paths = make_run_dirs(args.out_base, category="base/y", run_id=args.run_id)
    ...
    for dy in offsets:
        name = f"base_y_{dy:+.2f}m".replace("+", "p").replace("-", "m")
        ...
        for trial_idx, seed in enumerate(seeds):
            config = collector.setup_scene(
                ...
                panda_base_offset=[0.0, float(dy), 0.0],
            )
        ...
        print(f"[base/y] wrote {len(records)} -> {out_file}")
