...
def main() -> None:
    ...
    offsets = [0.0, 0.01, 0.02, 0.05]

    run_paths = make_run_dirs(args.out_base, category="extra/target/xy_offset", run_id=args.run_id)
    ...
    for d in offsets:
        name = f"target_dx_{int(d*100):02d}cm"
        out_file = run_paths.category_dir / f"{name}.jsonl"
        records = []

        for trial_idx, seed in enumerate(seeds):
            config = collector.setup_scene(randomize=True, seed=seed, object_spec=object_spec, target_xy_offset=[float(d), 0.0])
            if config is None:
                records.append({"category": "extra_target_offset", "condition": name, "trial": trial_idx, "seed": seed, "setup_ok": False, "success": False, "reason": "setup_failed", "target_xy_offset": [float(d), 0.0]})
                continue

            res = run_policy_episode(collector=collector, model=policy.model, stats=policy.stats, render=bool(args.render))

            records.append({
                "category": "extra_target_offset",
                "condition": name,
                "trial": trial_idx,
                "seed": seed,
                ...
                "reason": res.reason,
                "target_xy_offset": [float(d), 0.0],
            })

        write_jsonl(out_file, records)
        print(f"[extra/target/xy_offset] wrote {len(records)} -> {out_file}")
