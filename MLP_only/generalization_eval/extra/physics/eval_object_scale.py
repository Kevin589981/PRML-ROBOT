...
def main() -> None:
    ...
    run_paths = make_run_dirs(args.out_base, category="extra/physics/object_scale", run_id=args.run_id)
    ...
    for s in scales:
        ...
        for trial_idx, seed in enumerate(seeds):
            config = collector.setup_scene(
                ...
                object_scale=float(s),
            )

            if config is None:
                records.append({
                    "category": "extra_object_scale",
                    "condition": name,
                    "trial": trial_idx,
                    "seed": seed,
                    "setup_ok": False,
        ...
        print(f"[extra/physics/object_scale] wrote {len(records)} -> {out_file}")
