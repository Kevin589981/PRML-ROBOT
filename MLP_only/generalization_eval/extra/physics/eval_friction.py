...
def main() -> None:
    ...
    # 控制变量：只改摩擦（覆盖 cube 与 gripper 的 lateralFriction）
    object_spec = {"shape": "box", "half_extents": [0.025, 0.025, 0.025], "rgba": [0.85, 0.2, 0.2, 1.0]}

    frictions = [0.5, 1.0, 2.0, 3.0, 5.0]

    run_paths = make_run_dirs(args.out_base, category="extra/physics/friction", run_id=args.run_id)
    ...
    for mu in frictions:
        ...
        for trial_idx, seed in enumerate(seeds):
            config = collector.setup_scene(
                ...
                friction_override=float(mu),
            )

            if config is None:
                records.append({
                    "category": "extra_friction",
                    "condition": name,
                    "trial": trial_idx,
                    "seed": seed,
                    "setup_ok": False,
        ...
        print(f"[extra/physics/friction] wrote {len(records)} -> {out_file}")
