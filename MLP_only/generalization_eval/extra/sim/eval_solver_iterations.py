...
def main() -> None:
    ...
    run_paths = make_run_dirs(args.out_base, category="extra/sim/solver", run_id=args.run_id)
    ...
    for n in iters:
        ...
        for trial_idx, seed in enumerate(seeds):
            config = collector.setup_scene(
                ...
                solver_iterations=int(n),
            )

            if config is None:
                records.append({
                    "category": "extra_solver",
                    "condition": name,
                    "trial": trial_idx,
                    "seed": seed,
                    "setup_ok": False,
        ...
        print(f"[extra/sim/solver] wrote {len(records)} -> {out_file}")
