...
def main() -> None:
    ...
    scales = [0.8, 0.95, 1.0, 1.05, 1.2]

    run_paths = make_run_dirs(args.out_base, category="extra/sim/gravity", run_id=args.run_id)
    ...
    for s in scales:
        ...
        for trial_idx, seed in enumerate(seeds):
            config = collector.setup_scene(
                ...
                gravity_scale=float(s),
            )

            if config is None:
                records.append({
                    "category": "extra_gravity",
                    "condition": name,
                    "trial": trial_idx,
                    "seed": seed,
                    "setup_ok": False,
        ...
        print(f"[extra/sim/gravity] wrote {len(records)} -> {out_file}")
