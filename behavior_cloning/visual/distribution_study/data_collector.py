...
def _worker_task(seed, count, rand_config=None, record_video=False):
    col = VisualExpertDemoCollector(rand_config=rand_config, record_video=record_video)
    res = []
    attempts = 0
...
def run_collection(total=1000, workers=8, rand_config=None, record_video=False):
    ...
    with mp.get_context("spawn").Pool(workers) as pool:
        results = pool.starmap(
            _worker_task,
            [(i * 10000, per_w, rand_config, record_video) for i in range(workers)],
        )
    
    col = VisualExpertDemoCollector(rand_config=rand_config)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", type=str, default="data_enhanced/basket_demos_with_phase.h5")
    parser.add_argument("--total_episodes", type=int, default=1000)
    parser.add_argument("--workers", type=int, default=16)

    # ======== Distribution controls (control-variable study) ========
    # 1) Gripper end-effector offset distribution (expert's XY misalignment during approach/transport)
    parser.add_argument("--expert_alignment_noise", type=float, default=0.03)

    # 2) Basket initial position distribution (X only, +/- noise)
    parser.add_argument("--basket_pos_x_noise", type=float, default=0.08)

    # 3) Cube initial position distribution (x/y ranges)
    # We specify widths around the baseline centers (0.45, 0.025) to keep the mean fixed.
    parser.add_argument("--cube_x_width", type=float, default=0.40)
    parser.add_argument("--cube_y_width", type=float, default=0.75)
    parser.add_argument("--cube_x_center", type=float, default=0.45)
    parser.add_argument("--cube_y_center", type=float, default=0.025)

    # Optional: initial end-effector world-position noise at reset (kept default off)
    parser.add_argument("--ee_pos_noise", type=float, default=0.0)

    args = parser.parse_args()

    cube_x_min = args.cube_x_center - args.cube_x_width / 2
    cube_x_max = args.cube_x_center + args.cube_x_width / 2
    cube_y_min = args.cube_y_center - args.cube_y_width / 2
    cube_y_max = args.cube_y_center + args.cube_y_width / 2

    rand_config = RandomizationConfig(
        cube_pos_x_range=(cube_x_min, cube_x_max),
        cube_pos_y_range=(cube_y_min, cube_y_max),
        basket_pos_x_noise=float(args.basket_pos_x_noise),
        expert_alignment_noise=float(args.expert_alignment_noise),
        ee_pos_noise=float(args.ee_pos_noise),
    )

    print(
        "Using RandomizationConfig: "
        f"expert_alignment_noise={rand_config.expert_alignment_noise}, "
        f"basket_pos_x_noise={rand_config.basket_pos_x_noise}, "
        f"cube_x_range={rand_config.cube_pos_x_range}, "
        f"cube_y_range={rand_config.cube_pos_y_range}, "
        f"ee_pos_noise={rand_config.ee_pos_noise}"
    )
    ...
    if collector.trajectories:
        collector.save_dataset(args.save_path)