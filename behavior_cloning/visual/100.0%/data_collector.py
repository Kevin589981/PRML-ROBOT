...
class DataCollectionConfig:
    ...
    include_object_relative_pos: bool = True
...
class VisualExpertDemoCollector:
    ...
    def setup_scene(self, seed=None):
        ...
        self.basket_ids, self.target_pos = BasketBuilder.create_basket(
            self.data_config.basket_config, self.base_pos, self.table_height, self.client_id
        )
        ...
        b_w_pos = self.base_pos + self.data_config.basket_config.position_relative
    ...
    def collect_trajectory(self, seed=None) -> Optional[VisualTrajectory]:
        ...
        try:
            ...
            # Phase 1: Approach
            waypoints = [
                (c_pos + [0, 0, 0.18], 1.0, 0.020, 40, 10, 0.025, False),
                (c_pos + [0, 0, 0.12], 1.0, 0.015, 30, 10, 0.025, False),
                (c_pos + [0, 0, 0.06], 1.0, 0.012, 25, 8, 0.020, False),
                (c_pos + [0, 0, 0.025], 1.0, 0.008, 25, 8, 0.015, False),
                (c_pos + [0, 0, 0.005], 1.0, 0.006, 25, 8, 0.010, False),
                (c_pos + [0, 0, -0.005], 1.0, 0.005, 20, 8, 0.008, False),
            ]
            for wp in waypoints:
            ...
            waypoints = [
                ...
                (c_pos + [0, 0, 0.12], 0.0, 0.015, 25, 10, 0.025, True),
            ...
            # Phase 4: Transport
            move_target = self.target_pos + [0, 0, 0.22]
            dist = np.linalg.norm(move_target[:2] - c_pos[:2])
            frames = max(50, min(int(dist/0.04)+20, 120))
            _, dropped = self._move_to_target(move_target, 0.0, 0.025, frames, 12, 0.05, True, data, recorder, TaskPhase.TRANSPORT)
            if dropped: return None
            
            # Phase 5: Descend
            waypoints = [
                (self.target_pos + [0, 0, 0.15], 0.0, 0.015, 25, 8, 0.020, True),
                (self.target_pos + [0, 0, 0.10], 0.0, 0.012, 25, 8, 0.018, True),
                (self.target_pos + [0, 0, 0.06], 0.0, 0.010, 25, 8, 0.015, True),
            ...
            # Phase 7: Retreat
            waypoints = [
                (self.target_pos + [0, 0, 0.08], 1.0, 0.015, 20, 10, 0.020, False),
            ...
            # [关键修复] 参数顺序: phase在前, success(bool)在后
...
if __name__ == "__main__":
    mp.freeze_support()
    collector = run_collection(total=1000, workers=125, record_video=False) # 调整worker数量以适应你的CPU
