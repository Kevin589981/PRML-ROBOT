...
def main() -> None:
    ...
    for h in heights:
        name = f"cube_support_{int(h*100):02d}cm"
        ...
        for trial_idx, seed in enumerate(seeds):
            config = collector.setup_scene(
                ...
                cube_support_height=float(h),
                target_support_height=0.0,
            )

            if config is None:
                records.append({
                    "category": "heights_initial",
                    "condition": name,
                    "trial": trial_idx,
                    "seed": seed,
                    "setup_ok": False,
        ...
        print(f"[heights/initial] wrote {len(records)} -> {out_file}")
