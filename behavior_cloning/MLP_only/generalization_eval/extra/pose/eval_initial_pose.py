...
def _conditions() -> dict[str, dict]:
    # 单位：度。控制变量：一次只改变一个轴（yaw/pitch/roll）。
    degrees = [0, 5, 10, 20, 45]
    conds: dict[str, dict] = {}

    for d in degrees:
        conds[f"yaw_{d:02d}deg"] = {"cube_initial_euler_deg": [0.0, 0.0, float(d)]}
        conds[f"pitch_{d:02d}deg"] = {"cube_initial_euler_deg": [0.0, float(d), 0.0]}
        conds[f"roll_{d:02d}deg"] = {"cube_initial_euler_deg": [float(d), 0.0, 0.0]}
...
def main() -> None:
    ...
    run_paths = make_run_dirs(args.out_base, category="extra/pose", run_id=args.run_id)
    ...
    for name, cfg in conds.items():
        ...
        for trial_idx, seed in enumerate(seeds):
            config = collector.setup_scene(
                ...
                cube_initial_euler_deg=cfg["cube_initial_euler_deg"],
            )
        ...
        print(f"[extra/pose] wrote {len(records)} -> {out_file}")
