...
def main() -> None:
    ...
    timesteps = [1.0 / 480.0, 1.0 / 240.0, 1.0 / 120.0]

    run_paths = make_run_dirs(args.out_base, category="extra/sim/timestep", run_id=args.run_id)
    ...
    for dt in timesteps:
        name = f"dt_{int(round(1.0/dt))}hz"
        ...
        for trial_idx, seed in enumerate(seeds):
            config = collector.setup_scene(
                ...
                sim_time_step=float(dt),
            )

            if config is None:
                records.append({
                    "category": "extra_timestep",
                    "condition": name,
                    "trial": trial_idx,
                    "seed": seed,
                    "setup_ok": False,
        ...
        print(f"[extra/sim/timestep] wrote {len(records)} -> {out_file}")
