...
def main() -> None:
    ...
    # 控制变量：只改观测噪声（在归一化空间加噪）
    object_spec = {"shape": "box", "half_extents": [0.025, 0.025, 0.025], "rgba": [0.85, 0.2, 0.2, 1.0]}

    sigmas = [0.0, 0.01, 0.02, 0.05]

    run_paths = make_run_dirs(args.out_base, category="extra/noise/obs_noise", run_id=args.run_id)
    ...
    for s in sigmas:
        ...
        print(f"[extra/noise/obs_noise] wrote {len(records)} -> {out_file}")
